...
class bad_expected_access : bad_expected_access<void> {
 public:
    explicit bad_expected_access(const E& error) : error_(error) {}
...
class unexpected final {
 public:
    ...
    constexpr explicit unexpected(
        std::enable_if_t<std::is_convertible_v<Err, E>, Err>&& e)
        : error_(std::forward<Err>(e)) {}

    template <typename Err = E>
    constexpr explicit unexpected(
        const std::enable_if_t<std::is_convertible_v<Err, E>, Err>& e)
        : error_(e) {}
    ...
    */
    ...
    constexpr void swap(unexpected& e) noexcept(
...
class expected {
 public:
    ...
    constexpr explicit expected(const unexpected<error_type>& error)
        : data_(error.error()) {}
    constexpr explicit expected(unexpected<error_type>&& err)
    ...
    bool has_value() const { return std::get_if<value_type>(&data_); }
...
class expected<void, E> {
 public:
    ...
    template <typename U>
    constexpr expected(
        const std::enable_if_t<std::is_convertible_v<U, E>>& e)
        : error_(e) {}
    ...
    constexpr value_type value() { return; }
    ...
    constexpr error_type& error() & noexcept { return error_.value(); }
